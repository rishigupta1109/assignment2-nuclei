    ...
    order.forEach((field, ind) => {
      ...
      users.forEach((user, index) => {
        mockReadALine
          ...
          .mockReturnValueOnce(user.getName())
          .mockReturnValueOnce(user.getAge().toString())
          .mockReturnValueOnce(user.getAddress())
          .mockReturnValueOnce(user.getRollNumber().toString())
      ...
      mockReadALine
        ...
        .mockReturnValueOnce((index + 1).toString())
        .mockReturnValueOnce((ind + 1).toString());
...
test('testing sortUsers function with invalid field', () => {
  ...
  expect(() => userDataManagement.sortUsers(1, 5)).toThrowError(
    'Invalid Field',
  );
  expect(() => userDataManagement.sortUsers(5, 5)).toThrowError(
    'Invalid Order',
  );
});
test('testing add user function', () => {
  ...
  mockReadALine.mockReturnValue('1');
  userDataManagement.run();
  expect(userDataManagement.displayUsers()).toEqual([user1, user3, user2]);
});
...
test('testing add user with akready existing roll number', () => {
  ...
  userDataManagement.run();
  expect(() => userDataManagement.addUser()).toThrowError(
    'User with this roll number already present.',
  );
});
test('testing delete user function', () => {
  ...
  mockReadALine.mockReturnValue('1');
  userDataManagement.run();
  expect(userDataManagement.displayUsers()).toEqual([user3, user2]);
});
